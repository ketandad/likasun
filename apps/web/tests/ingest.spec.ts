...
test('upload and parse CSV', async ({ page }) => {
  await page.route('**/ingest/files', (route) =>
    route.fulfill({ json: { upload_ids: { 'aws_s3_inventory.csv': '1' } } }),
  );
...
test('validate permissions shows missing', async ({ page }) => {
  await page.route('**/ingest/live/permissions?cloud=aws', (route) =>
    route.fulfill({ json: { 's3:ListBuckets': true, 'ec2:Describe': false } }),
  );
  await page.goto('/ingest');
...
test('start live ingestion', async ({ page }) => {
  await page.route('**/ingest/live?cloud=aws', (route) =>
    route.fulfill({ json: { ingested: 12, errors: [] } }),
  );
  await page.goto('/ingest');
